import math
import numpy as np
from tp_RFeatures import builder_features
...
class DiscModel(object): # Discretisation for non uniform polar discretisation
	def __init__(self,dictionary,actions,kinematics = kinematics_builder,feature = builder_features):
		self.feature = feature
		self.actions = actions#these are in the form of a dictionary. but they really should not be TODO
		self.state_names = dictionary.keys()
		self.bin_info = np.array(dictionary.values())
		print self.bin_info
...
if __name__=="__main__":
	...
	m = DiscModel(state_dictionary,actions)
	print m.stateToQuantity(56)
	#m.indexToAction(0)
	#m.indexToAction(0)
